(() => {
  ...
  function rebuildVisibleQuestions(preferredId = null) {
    ...
    const wantedIndex = preferredId
      ? questions.findIndex(question => question.id === preferredId)
      : -1;

    state.index = wantedIndex >= 0
      ? wantedIndex
      : Math.min(state.index, Math.max(questions.length - 1, 0));
  ...
  function applySearch() {
    ...
    if (!bank) {
      renderSearchResults();
      setStatus("当前没有可搜索的题库。", true);
      return;
    }

    const normalizedTerm = term.toLocaleLowerCase();
    let searchScope = [...bank.questions];

    if (state.mode === "wrong") {
      const wrongSet = new Set(getBankProgress().wrongIds);
      searchScope = searchScope.filter(question => wrongSet.has(question.id));
    }

    state.searchResults = searchScope
      .map((question, originalIndex) => {
        const questionText = question.question.toLocaleLowerCase();
        const optionText = Object.values(question.options).join("\n").toLocaleLowerCase();
        const sourceText = question.source.toLocaleLowerCase();
        const explanationText = question.explanation.toLocaleLowerCase();

        let score = 0;
        if (questionText.includes(normalizedTerm)) score += 100;
        if (questionText.startsWith(normalizedTerm)) score += 30;
        if (optionText.includes(normalizedTerm)) score += 40;
        if (sourceText.includes(normalizedTerm)) score += 20;
        if (explanationText.includes(normalizedTerm)) score += 10;

        return { question, score, originalIndex };
      })
    ...
    if (state.searchResults.length === 0) {
      setStatus(`没有找到与“${term}”相关的题目。`, true);
      return;
    }

    setStatus(`找到 ${state.searchResults.length} 道与“${term}”相关的题目，请从列表中选择。`);
  }
  ...
  function renderSearchResults() {
    ...
    title.textContent = state.searchResults.length
      ? `搜索结果：${state.searchResults.length} 道题`
      : "没有找到相关题目";

    const description = document.createElement("span");
    description.textContent = state.mode === "wrong"
      ? "当前搜索范围：错题本"
      : "当前搜索范围：全部题目";

    header.append(title, description);
    el.searchResults.appendChild(header);

    if (state.searchResults.length === 0) {
      const empty = document.createElement("p");
      empty.className = "search-empty";
      empty.textContent = `没有找到与“${term}”相关的题目。`;
    ...
    state.searchResults.forEach((question, resultIndex) => {
      ...
      questionTitle.className = "search-result-title";
      questionTitle.textContent = question.question;

      const meta = document.createElement("span");
      meta.className = "search-result-meta";
      const typeText = question.type === "multiple" ? "多选题" : "单选题";
      meta.textContent = `${typeText}${question.source ? ` · ${question.source}` : ""}`;

      const action = document.createElement("span");
      action.className = "search-result-action";
      action.textContent = "选择此题";

      content.append(questionTitle, meta);
      button.append(number, content, action);
      button.addEventListener("click", () => selectSearchResult(question.id));
  ...
  function selectSearchResult(questionId) {
    rebuildVisibleQuestions(questionId);

    const selectedIndex = state.visibleQuestions.findIndex(question => question.id === questionId);
    if (selectedIndex < 0) {
      setStatus("无法打开这道题，它可能已不在当前模式中。", true);
      return;
    }
    ...
    render();
    scrollToQuestion();
    setStatus(`已打开搜索结果中的第 ${selectedIndex + 1} 题，可以开始回答。`);
  }

  function scrollToQuestion() {
    const questionCard = el.questionText.closest(".question-card");
